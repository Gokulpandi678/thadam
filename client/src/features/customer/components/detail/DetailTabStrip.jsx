...
const DetailTabStrip = ({ tabs, active, onChange }) => (
  <div className="flex gap-1 border-b border-slate-100 mb-5 overflow-x-auto scrollbar-none -mx-1 px-1">
    {tabs.map((tab) => (
      <button
        key={tab.key}
        type="button"
        onClick={() => onChange(tab.key)}
        className={`flex items-center gap-1.5 px-3 sm:px-4 py-2.5 text-[13px] font-medium border-b-2 -mb-px transition-colors cursor-pointer whitespace-nowrap ${
          active === tab.key
            ? "text-blue-600 border-blue-600"
            : "text-slate-400 border-transparent hover:text-slate-600"
        }`}
