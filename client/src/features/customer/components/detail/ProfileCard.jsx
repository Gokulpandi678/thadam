...
const ProfileCard = ({ customer, profileCompletion, onEdit, onConvertAsClient, onDelete }) => {
  ...
  return (
    <>
      <div className="flex flex-col gap-2 items-center pt-7 px-4 sm:px-5 pb-5 bg-white rounded-lg shadow-[0_1px_3px_rgba(0,0,0,0.06),0_4px_16px_rgba(0,0,0,0.04)] relative">
        <div className="absolute top-0 left-0 right-0 h-18 bg-linear-to-br from-blue-600 to-blue-500 rounded-t-lg" />
        ...
        <p className="text-xs font-bold text-slate-700 mt-1 mb-4 sm:mb-5 text-center">{formattedCompanyDetail}</p>

        <div className="w-full flex flex-col gap-3 mb-4 sm:mb-5">
          <InfoRow
            icon={<Mail size={15} />}
            label="Email"
            value={customer.primaryEmail}
        ...
        <div className="flex gap-2 w-full">
          <button
            onClick={onEdit}
            aria-label="Edit contact"
            className="w-10 h-10 sm:w-9.5 sm:h-9.5 flex-none rounded-[10px] border border-blue-100 bg-blue-50 flex items-center justify-center cursor-pointer text-blue-500 transition-all duration-150 hover:bg-blue-500 hover:text-white shrink-0"
          >
            <Pencil size={15} />
          </button>

          {canConvert && (
            <TextButton onClick={onConvertAsClient} className="flex-1 min-w-0 text-[13px] sm:text-[13px]">Convert as Client</TextButton>
          )}

          <button
            onClick={onDelete}
            aria-label="Delete contact"
            className="w-10 h-10 sm:w-9.5 sm:h-9.5 flex-none rounded-[10px] border border-red-100 bg-red-50 flex items-center justify-center cursor-pointer text-red-500 transition-all duration-150 hover:bg-red-500 hover:text-white shrink-0"
          >
            <Trash2 size={15} />
          </button>
        </div>
