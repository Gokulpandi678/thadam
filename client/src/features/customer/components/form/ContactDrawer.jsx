...
import { isClientContact } from "../../../../utils/customer.utils";

const getVisibleTabs = ({ isEdit, contact, form, tabs }) => {
  if (!isEdit) return tabs;

  const shouldShowClientTab = form?.role === "Client" || isClientContact(contact);
  const hasCustomMeetingOnlyTabs = tabs?.every((tab) => tab.label === "Meeting");

  return tabs.filter((tab) => {
    if (tab.label === "Meeting" && !hasCustomMeetingOnlyTabs) return false;
    if (tab.label === "Client" && !shouldShowClientTab) return false;
    return true;
  });
};
...
const ContactDrawer = ({
  ...
  const ctaLabel = submitLabel ?? (isEdit ? "Save changes" : "Save contact");
  const activeTabs = getVisibleTabs({
    isEdit,
    contact,
    form,
    tabs: tabs ?? contactDrawerTabs,
  });
  const headerTitle = title ?? (isEdit ? `Edit contact - ${fullName}` : "New contact");
  const headerSubtitle = subtitle ?? (
    isEdit && contact
      ? [contact.designation, contact.company, contact.role].filter(Boolean).join(" | ")
      ...
      >
        <div className="bg-blue-600 px-6 py-5 flex items-center gap-3 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {headerSubtitle && (
              <p className="text-blue-200 text-[12px] truncate">{headerSubtitle}</p>
            )}
          </div>
        ...
        <DrawerForm tabs={activeTabs} form={form} errors={errors} onChange={onChange} />
