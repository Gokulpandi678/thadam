...
public class AuthController {
	...
	public Response callback(@QueryParam("code") String code, @QueryParam("state") String state,
		...
		NewCookie accessCookie = new NewCookie.Builder(ACCESS_TOKEN_COOKIE).value(accessToken).path("/").httpOnly(true)
				.secure(true).sameSite(NewCookie.SameSite.NONE).maxAge(ACCESS_TOKEN_MAX_AGE).build();

		NewCookie refreshCookie = new NewCookie.Builder(REFRESH_TOKEN_COOKIE).value(refreshToken).path("/")
				.httpOnly(true).secure(true).sameSite(NewCookie.SameSite.NONE).maxAge(REFRESH_TOKEN_MAX_AGE).build();

		return Response.seeOther(URI.create(sysProps.getWebSuccessRedirect())).cookie(accessCookie)
	...
	public Response refresh(@CookieParam(REFRESH_TOKEN_COOKIE) String refreshToken) {
		...
		NewCookie accessCookie = new NewCookie.Builder(ACCESS_TOKEN_COOKIE).value(refreshed.getAccessToken()).path("/")
				.httpOnly(true).secure(true).sameSite(NewCookie.SameSite.NONE).maxAge(ACCESS_TOKEN_MAX_AGE).build();

		GenericResponse successMessage = authService.buildSuccessResponse("Token refreshed");
		return Response.ok().cookie(accessCookie).entity(successMessage).build();
	}
	...
	public Response logout(@CookieParam(ACCESS_TOKEN_COOKIE) String accessToken) {
		...
		try {
			...
			NewCookie deleteAccess = new NewCookie.Builder(ACCESS_TOKEN_COOKIE).value("").path("/").maxAge(0)
					.httpOnly(true).secure(true).build();

			NewCookie deleteRefresh = new NewCookie.Builder(REFRESH_TOKEN_COOKIE).value("").path("/").maxAge(0)
					.httpOnly(true).secure(true).build();

			return Response.seeOther(URI.create(workOSLogoutUrl)).cookie(deleteAccess).cookie(deleteRefresh).build();
