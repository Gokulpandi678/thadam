...
public class CustomerController {
	...
	public RestResponse<GenericResponse> getCustomers(@QueryParam("search") String search,
			@QueryParam("role") List<String> roles, @QueryParam("designation") List<String> designations,
			@QueryParam("city") List<String> cities, @DefaultValue("0") @QueryParam("page") int page) {

		return customerService.getCustomers(requestContext.getUserId(), search, roles, designations, cities, page);
	}
	...
	public RestResponse<GenericResponse> updateCustomer(@PathParam("id") UUID id, @Valid CustomerRequest request) {
	...
	public RestResponse<GenericResponse> addLog(@PathParam("customerId") UUID customerId,
	...
	public RestResponse<GenericResponse> editLog(@PathParam("customerId") UUID customerId,
			@PathParam("logId") UUID logId, @Valid CustomerLogRequest request) {
		return customerService.editLog(customerId, logId, request, requestContext.getUserId());
	}

	@DELETE
	@Path("/{customerId}/deleteLog/{logId}")
	public RestResponse<GenericResponse> deleteLog(@PathParam("customerId") UUID customerId,
	...
	public RestResponse<GenericResponse> addProfilePic(@PathParam("id") UUID id, @RestForm("file") FileUpload file) {
	...
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	public RestResponse<GenericResponse> editProfilePic(@PathParam("id") UUID id, @RestForm("file") FileUpload file) {
